 ...
 * 测量文字的宽度并返回。
 * <p>
 * 如果你用代码分别使用 getTextBounds() 和 measureText() 来测量文字的宽度，你会发现  measureText() 测出来的宽度总是比 getTextBounds() 大一点点。
 ...
 *
 * measureText():
 * 它测量的是文字绘制时所占用的宽度（关键词：占用）。
 * 前面已经讲过，一个文字在界面中，往往需要占用比他的实际显示宽度更多一点的宽度，以此来让文字和文字之间保留一些间距，不会显得过于拥挤。
 * 上面的这幅图，我并没有设置 setLetterSpacing() ，这里的 letter spacing 是默认值 0，但你可以看到，图中每两个字母之间都是有空隙的。另外，下方那条用于表示文字宽度的横线，在左边超出了第一个字母 H 一段距离的，在右边也超出了最后一个字母 r（虽然右边这里用肉眼不太容易分辨），而就是两边的这两个「超出」，导致了 measureText() 比 getTextBounds() 测量出的宽度要大一些。
 *
 */
public class Practice12MeasureTextView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float a = paint1.measureText(text1);
        float b = paint2.measureText(text2);
        float c = paint1.measureText(text3);
        canvas.drawText(text1, 50, 200, paint1);
        canvas.drawText(text2, 50 + a, 200, paint2);
        canvas.drawText(text3, 50 + a + b, 200, paint1);
    }
}
